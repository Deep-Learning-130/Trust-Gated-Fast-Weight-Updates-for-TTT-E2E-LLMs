...
(Section~\ref{sec:protocol}). Section~IV reports what has been executed against
that protocol, and Section~V concludes.
...
\label{prop:drift}
Let $\fast_{\mathrm{ref}}$ denote the fast weights at the start of a budget
window --- the anchor $\anchorp$ at the beginning of a request, or the last
trusted checkpoint after a reset. Then for every window $w$ within that budget
window,
\begin{equation}
...
\subsubsection{Implementation status of the bound}
\label{sec:driftstatus}
Proposition~\ref{prop:drift} is a statement about the gated commit
\eqref{eq:gatedcommit}, and it holds wherever that commit is the one being
executed. In our own overlay it is not yet enforced, and we state this here rather
than leave a reader to infer that a proved bound is a running one. The accumulator
of \eqref{eq:accumulate}, the latch of \eqref{eq:latch} and the versioned store of
Section~\ref{sec:store} are implemented and unit-tested, but they are not reachable
from the interceptor, and the obstruction is structural rather than a matter of
effort. The reference implementation's inner loop runs under a compiled scan whose
carry is fixed to the model, the inner optimizer state and the recurrent and suffix
state; a scan requires the structure of the carry out to match the carry in, so
there is no slot in which a cumulative drift scalar and a breach latch can ride
from one window to the next without altering the vendor's own carry signature ---
which is precisely what an overlay is not permitted to do. The route we have
identified threads the accumulator through the inner optimizer state, which is
already a carry slot; we have specified that route and deliberately written no code
for it, because doing so before the criterion of Section~\ref{sec:prereg} is
discharged would invert the ordering this protocol exists to protect. What is
therefore demonstrated in this paper is the per-update path --- interception,
anchor consistency, uncertainty, and the accept/reject decision --- and what
remains specified but unenforced is the cumulative bound and the rollback that
depends on it.
...
\label{sec:sut}
Experiments use the released TTT-E2E $125$M checkpoint, DCLM-pretrained and
fine-tuned on Books at an $8$K context~\cite{tandon2025tttE2E}, in its reference JAX
implementation~\cite{bradbury2018jax,kidger2021equinox}, on a single $8$\,GB
consumer GPU; a $16$\,GB hosted accelerator without native \texttt{bfloat16} is the
named fallback, in which case both arms run at \texttt{float32} and the dtype is
recorded with the result. The model is run in its meta-learning mode, which is the
only mode with an inner loop; in pretraining mode there are no fast weights to
poison and the attack is vacuously null.

This is a change of scale from the configuration originally pre-registered, which
named the $1$B checkpoint on a single $80$\,GB accelerator, and we record it as a
dated revision rather than a silent substitution. The $1$B configuration has no
hardware: a quota request for an $80$\,GB card was refused, and the alternative
provider available to us required roughly a day of credit deposited in advance
under payment methods we could not use. No threshold in Table~\ref{tab:prereg}
moved with the scale. Two consequences are fixed in advance. The environment
soundness check of Section~\ref{sec:soundness} weakens, because the source paper
publishes no held-out loss for $125$M at $8$K and there is therefore no numeric
bracket to fall inside; only the structural checks apply, and a pass must not be
read as a reproduction. And a result at $125$M is a result at $125$M: a corruption
demonstrated there is an existence proof at that scale and makes no claim about
$1$B, while a null there may be a null only at that scale.

Context length is a first-order experimental variable, not a detail. At $8$K the
sliding-window attention spans the entire sequence and the fast weights receive
only $8$ inner updates, so a defense evaluated there is evaluated where the
mechanism it defends barely operates. The committed configuration is the released
checkpoint at its native $8$K, and every claim below is scoped to that.

Table~\ref{tab:baselines} and Fig.~\ref{fig:baselines} place TTT-E2E against six
prior long-context methods, as published by its
...
bottleneck, all on held-out Books loss at five context lengths, at 3B parameters
and $3\times$ Chinchilla compute. These are published values, not our
measurements, and we recovered them from the source paper's vector figure data
instead of reading them off by eye. Pairing the axis tick stubs with their labels
inside the figure's own coordinate space gives a linear calibration whose maximum
residual is $4\times10^{-16}$ nats. We then cross-validated every value against a
second figure that plots the same runs as differences from full attention; all 35
agree to four decimal places. Three further constraints corroborate the reading.
At 8K context with window $k=8\text{K}$, sliding-window attention and the hybrid
\emph{are} full attention, and all three curves do read identically. Every value
lands on the 0.001 grid that the source prints elsewhere. And every qualitative
claim in the source captions is reproduced.

\input{figures/table1_baselines}
\input{figures/fig1_baselines}

One number from Table~\ref{tab:baselines} constrains what any corruption
measurement can mean, and it is why the table appears here rather than among our
own results. At 8K context the entire benefit of test-time training over
sliding-window attention is $2.328 - 2.314 = 0.014$ nats. That margin is published
at 3B and the system under test here is $125$M, so it is an indication of the
mechanism's headroom rather than a figure for our configuration; we return to it in
Section~\ref{sec:prereg}, where the degradation bar is set against it.
...
is certain. That bracket is a $1$B quantity. At the $125$M scale of
Section~\ref{sec:sut} the source publishes no corresponding value, so no numeric
bracket is available and the structural checks below carry the whole weight of this
step. We are explicit about what this establishes and what it does not. It
...
The degradation bar deserves to be read against the margin recorded in
Section~\ref{sec:sut}, because the two are not independent and the comparison is
unflattering. Ten per cent of a ${\sim}2.3$-nat baseline is ${\sim}0.23$ nats,
roughly sixteen times the $0.014$ nats that test-time training buys over
sliding-window attention at 8K. We state the asymmetry where the bar is set rather
than discovering it beside a result. It does not make the bar impossible ---
corruption may exceed benefit, since breaking is easier than helping and the
attacker optimises while the mechanism does not --- but it does mean the bar is
demanding at the cheapest context length, which is also the length at which the
mechanism does the least work. Any negative result under this protocol is scoped
to 8K and to the eight inner updates a single sequence provides, and is not a claim
that fast weights are unpoisonable.
