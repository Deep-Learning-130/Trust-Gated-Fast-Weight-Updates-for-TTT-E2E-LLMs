...
2. **CPU tests pass:** `PYTHONPATH=src JAX_PLATFORMS=cpu pytest` — as of 2026-09-14,
   **249 passed / 11 skipped** with no reference model fetched (what CI sees), and
   **260 passed** once `scripts/fetch_reference_model.py` has run. The skips are the opt-in
